...
/*
  find the smallest key > input key
  key = 9 => 11
  key = 14 => 20
  key = 12 => 14
  key = 25 => null
  
  create a temp = Infinity

  create a recursive function
    if this node doesnt exist return
    if this node is > than the key
      flag as visited
      if this node is less than temp, make it the new temp
      call on this node
    if node left
      call on node left
    if node right call on node right
    if (!this.parent.visted) {
      call on parent
    }
*/
...
BinarySearchTree.prototype.findInOrderSuccessor = function(inputNode) {
  ...
  const search = node => {
    ...
    if (node.key > inputNode.key) {
      this.visited = true;
      temp = node.key > temp ? temp : node.key;
      search(node);
    }
    if (node.left) {
      search(node.left);
    ...
    if (this.parent && !this.parent.visited) {
      search(this.parent);
    }
  }
  search(inputNode);
